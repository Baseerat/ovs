...
// @P4:
#include "p4/src/action/dpif-netdev.h"
...
static struct dp_packet *dpif_netdev_compose_probe_pkt(const struct eth_addr src_mac, const struct eth_addr dst_mac,
...
//        if (!(data >= (thresh_val-thresh) && data <= (thresh_val+thresh))) {
    if (!probe_pkt) {
      probe_pkt = dpif_netdev_compose_probe_pkt(src_mac, dst_mac, src_ip, dst_ip, data);
    } else {
